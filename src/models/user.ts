...
export class UserStore {
  ...
  async index(): Promise<User[]> {
    try {
      ...
      const sql = 'SELECT id,username, first_name, last_name,password FROM users;';
  ...
  async update(user: User): Promise<User>{
    try{
      const connection = await Client.connect();
      const sql = `UPDATE users SET username=$1,first_name=$2,last_name=$3,password=$4
       WHERE id=$6
      ...
      throw new Error(`Could't update user ${user.username}: ${err}`);
    }
  }
  ...
  async authenticate(
    ...
    try {
      ...
      throw new Error(`Unable to login ${username}: ${err}`);
    }
  }
